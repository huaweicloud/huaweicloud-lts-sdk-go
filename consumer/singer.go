...
func SignHeaderBasic(r *http.Request, ak string, sk string, derivedAuthServiceName string, regionId string, queryMap map[string]string) (map[string]string, error) {
	...
	canonicalRequest, err := CanonicalRequestBasic(r, signedHeaders, queryMap)
	if err != nil {
		return nil, err
	}
...
func CanonicalRequestBasic(r *http.Request, signedHeaders []string, queryMap map[string]string) (string, error) {
	...
	return fmt.Sprintf("%s\n%s\n%s\n%s\n%s\n%s",
		r.Method,
		CanonicalURIBasic(r),
		CanonicalQueryStringBasic(queryMap),
...
func CanonicalQueryStringBasic(queryMap map[string]string) string {
	...
	for _, value := range keyValues {
		queryStrings = append(queryStrings, encode(value)+"="+encode(queryMap[value]))
	}
...
func encode(value string) string {
	...
	strings.ReplaceAll(encodeValue, "+", "%20")
	strings.ReplaceAll(encodeValue, "*", "%2A")
	strings.ReplaceAll(encodeValue, "%7E", "~")
	return encodeValue
}
